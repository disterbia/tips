...
// @Tags 랜딩페이지 /landing
// @Summary adapfit 장비 구매상담
// @Description adapfit 장비 구매상담 등록시 호출
// @Accept  json
// @Produce  json
// @Param request body AdapfitInquireReqeust true "요청 DTO - 신청서 데이터"
// @Success 200 {object} BasicResponse "성공시 200 반환"
// @Failure 400 {object} ErrorResponse "요청 처리 실패시 오류 메시지 반환"
// @Failure 500 {object} ErrorResponse "요청 처리 실패시 오류 메시지 반환"
// @Router /adapfit/inquire [post]
func AdapfitInquireHandler(endpoint endpoint.Endpoint) fiber.Handler {
	return func(c *fiber.Ctx) error {

		var req AdapfitInquireReqeust
		if err := c.BodyParser(&req); err != nil {
			return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{"error": err.Error()})
		}
